...
class Category {
    ...
    private $orders=NULL;                                           //字段映射，分类id，上级分类fid,分类名称name,格式化后分类名称fullname
    ...
    private function _findAllCat($condition, $orderby = NULL) {
        $this->rawList = empty($orderby) ? $this->model->where($condition)->order($this->orders)->select() : $this->model->where($condition)->order($orderby)->select();
    }
    ...
    /**
     * 递归组合为一维数组，
     * $html 子级填充的符号，$pid 父级id, $level 等级
     */
    static public function unlimitedForLevel($cate,$pid=0,$level=0){
      $arr = array();
      foreach($cate as $v){
        if($v['parent_id'] == $pid){
          $v['level'] = $level+1;
          // $v['html'] = str_repeat($html, $level);   //根据等级填充子级标识符
          $arr[] = $v;
          $arr = array_merge($arr,self::unlimitedForLevel($cate,$v['term_id'],$level+1));
        }
      }
      return $arr;
    }
    ...
    public function unlimitedForLayer($cate,$pid=0){
      ...
      foreach($cate as $v){
        if($v['parent_id'] == $pid){
          $v['child'] = self::unlimitedForLayer($cate,$v['term_id']);
          $arr[] = $v;
        }
      }
      return $arr;
     ...
     * 根据子级中的父id，获取所有父级
     */
    public function getParents($cate,$pid){
      $arr = array();
      foreach($cate as $v){
        if($v['term_id'] == $pid){
          $arr[] = $v;
          // $arr = array_merge($arr,$this->getParents($cate,$v['parent_id']));

          //将父级排列在前面，
          $arr = array_merge($this->getParents($cate,$v['parent_id']),$arr);
        }
      }
      return $arr;
    }
    ...
    public function getChildsId($cate,$id){
      ...
      foreach($cate as $v){
        ...
        if($v['parent_id'] == $id){
          $arr[] = $v['term_id'];    //将子类id加入数组
          $arr = array_merge($arr,$this->getChildsId($cate,$v['term_id']));
        }
      }
      return $arr;
    }
     ...
     * 传入父级id号，获取所有子级
     */
    public function getChilds($cate,$id){
      $arr = array();
      foreach($cate as $v){
        if($v['parent_id'] == $id){
          $arr[] = $v;    //将子级加入数组
          $arr = array_merge($arr,$this->getChilds($cate,$v['term_id']));
        }
      }
      return $arr;
    }
